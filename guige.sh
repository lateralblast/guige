...
# Function: do_exit
#
# Selective exit (don't exit when we're running in testmode mode)
...
do
  case "${1}" in
    ...
    --dockerworkdir)
      ...
      ;;
    --dryrun|--test*)
      # Run in dry-run mode - will not execute any commands
      options_list+=(testmode)
      shift
      ;;
    --efi)
      # EFI boot type
    ...
    --netmask)
      ...
      ;;
    --nfs)
      # Use NFS for boot server URL
      iso['bootserverprotocol']="nfs"
      shift
      ;;
    --nic)
    ...
    --shell)
      ...
      ;;
    --smb)
      # Use SMB for boot server URL
      iso['bootserverprotocol']="smb"
      shift
      ;;
    --sourceid)
      # Source ID
