    ...
    if [ "$TEST_MODE" = "false" ]; then
      ...
      for ISO_DEVICE in $ISO_DEVICES; do
        for ISO_VOLMGR in $ISO_VOLMGRS; do
          ...
          if [ "$ISO_HWE_KERNEL" = "true" ]; then
            echo "  linux   /casper/hwe-vmlinuz $ISO_KERNEL_ARGS quiet autoinstall fsck.mode=skip ds=nocloud\;s=$ISO_INSTALL_MOUNT/$ISO_AUTOINSTALL_DIR/configs/$ISO_VOLMGR/$ISO_DEVICE/  ---" >> "$ISO_SOURCE_DIR/boot/grub/grub.cfg"
  ...
  for ISO_DEVICE in $ISO_DEVICES; do 
    for ISO_VOLMGR in $ISO_VOLMGRS; do
      if [ -f "$WORK_DIR/$ISO_VOLMGR/$ISO_DEVICE/user-data" ]; then
        ...
        handle_output "echo \"    - \\\"echo 'dpkg --auto-deconfigure --force-depends -i /var/postinstall/packages/*.deb' >> $ISO_TARGET_MOUNT/tmp/post.sh\\\"\" >> \"$CONFIG_DIR/$ISO_VOLMGR/$ISO_DEVICE/user-data\""
        ...
        handle_output "echo \"    - \\\"curtin in-target --target=$ISO_TARGET_MOUNT -- ln -s /etc/localtime /usr/share/zoneinfo/$ISO_TIMEZONE\\\"\" >> \"$CONFIG_DIR/$ISO_VOLMGR/$ISO_DEVICE/user-data\""
        ...
        if [ "$TEST_MODE" = "false" ]; then
          ...
          echo "    - \"echo 'dpkg --auto-deconfigure --force-depends -i /var/postinstall/packages/*.deb' >> $ISO_TARGET_MOUNT/tmp/post.sh\"" >> "$CONFIG_DIR/$ISO_VOLMGR/$ISO_DEVICE/user-data"
          ...
          echo "    - \"curtin in-target --target=$ISO_TARGET_MOUNT -- ln -s /etc/localtime /usr/share/zoneinfo/$ISO_TIMEZONE\"" >> "$CONFIG_DIR/$ISO_VOLMGR/$ISO_DEVICE/user-data"
