...
reset_defaults () {
  ...
  if [[ "$ISO_OS_NAME" =~ "rocky" ]]; then
    ...
    DEFAULT_CI_INPUT_FILE="$DEFAULT_WORK_DIR/$DEFAULT_ISO_REALNAME-$DEFAULT_ISO_RELEASE-server-cloudimg-$DEFAULT_ISO_ARCH.img"
...
set_default_flags () {
  ...
  DO_CHECK_ISO="false"
  DO_CREATE_CI_VM="false"
  DO_DELETE_CI_VM="false"
  DO_CREATE_ISO_VM="false"
