...
get_interactive_input () {
  # NEW_options must be declared associative: without this, bash treats
  # NEW_options['dhcp'] etc. as an indexed array with an arithmetic (and
  # thus always-0) subscript, so every option answer below collides into
  # the same index and overwrites the previous one.
  ...
  if [ "${options['createexport']}" = "true" ] || [ "${options['createansible']}" = "true" ]; then
    ...
    # Get Architecture
    read -r -p "Architecture [${iso['arch']}]: " new['arch']
    ...
    iso['kernelargs']="${new['kernelargs']:-${iso['kernelargs']}}"
    # Get Disk Size
