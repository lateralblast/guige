...
- Fixed `options['unmount']` defaulting to `true` and running `unmount_iso`/`unmount_squashfs` for actions that never mounted anything (e.g. `--action printenv`/`checkworkdir`), and forcing `options['help']` false so the help fallback could never fire; now gated on whether `runchrootscript`/`createautoinstall`/`justiso` actually ran
- Fixed the `debug`/`verbose` early-detection at the top of `guige.sh` substring-matching the whole raw argument line, so `--hostname debugbox` or `--options noverbose` unintentionally enabled `set -x`/`set -eu`; replaced with a proper token match. While fixing this, found and fixed a **second** instance of a bug fixed in `5.0.3`: `get_switches`/`print_cli_help` mistaking an early comment or code for a switch definition because it merely contained `--something`; both scanners now ignore everything before the real `case "${1}" in` line, which should prevent this class of bug from recurring regardless of what gets added near the top of the file in future
- Fixed `check_value`'s "looks like a parameter" guard: it referenced an undefined `do_force` (so `--options force` could never suppress it) and flagged any value merely *containing* `--` (e.g. kernel args) instead of values that start with it
- Fixed password hashing on macOS using `openssl sha512` (a plain digest) instead of `openssl passwd -6` (an actual crypt(3) hash), so generated passwords never actually worked as login passwords
- Fixed several subiquity autoinstall generation no-ops and bugs: `--swap`/swap-size never created an LVM swap volume; the nvidia-blacklist-aware vfio module file used the wrong iso field for its own filename; the `--grublayout` deferred-substitution stanza used the YAML key `package:` instead of `layout:`; the grub-password deferred-substitution `sed` used `/` as its delimiter, which breaks when the generated password hash itself contains `/`; the btrfs-compression post-install script overwrote (instead of appended to) any late-packages install step already written to the same file; late packages were never actually executed for `xfs` or non-compressed `btrfs`; and desktop-build detection compared against the literal string `"desktop"`, which `iso['build']` never equals (it's always `live/desktop`/`daily/desktop`), so desktop builds incorrectly got the server-only apt/kernel/keyboard config sections
- Fixed `check_kvm_user` reading the wrong `/etc/group` field and comparing the group name against `$USER` instead of checking group membership, so it ran `usermod` unconditionally on every invocation; fixed the KVM stop-before-delete check counting "shut off" VMs system-wide instead of querying the target VM's own state via the connection actually used to manage it
- Fixed `delete_dir` calling `rm -f` on a directory (always fails; needs `-r`)
- Fixed the output cloud-image path using `/file/` instead of `/files/`, and reusing the exact input filename, which risked the output silently overwriting the input; it now writes a distinctly-named file into the correct directory, matching the existing default-file-naming convention
- Fixed `check_ipmitool`'s installed-check: `grep -c` always outputs a count (even "0"), so `-z` on its output was never true and the "ipmitool not found" warning could never fire
- Fixed two undefined-variable reads in `switches.sh`: `current['release9']` (Rocky `--release 9` resolved to an empty release) and `defaults['vmtype']` (an ISO-VM's kernel field was left empty)
- Fixed `process_actions` unconditionally resetting `options['force']`/`options['forceall']` to `false` for any action other than a delete, silently undoing an earlier `--options force`; also fixed an error message that printed the wrong (empty) variable for an invalid action name
- Fixed `process_post_install`'s default (`--postinstall none`, the default) forcibly resetting `installpackages`/`networkupdates`/`packageupdates`/`packageupgrades`/`distupgrade` to `false` even when the user had separately requested them via `--options`
- Fixed the `--action listswitches` output getting `--help` text appended after it — a regression from this same round's `unmount` fix, which had been silently relying on the `unmount` bug to force `options['help']` false; `listswitches` now exits on its own once it's done, matching every other list/print-and-stop action
